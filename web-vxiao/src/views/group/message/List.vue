...
<script>
  ...
  export default {
    ...
    activated() {
      let cache = this.getStorage('_message_cache')
      let refresh = this.getStorage('_message_list_refresh')
      if (refresh) {
        this.initData()
        this.loadMessages()
      } else if (cache) {
        this.messages.forEach((v, i) => {
          if (cache.id === v.id) {
            let praises = ArrayUtils.update(v.praises || [], cache.praises || [])
            let replys = ArrayUtils.update(v.replys || [], cache.replys || [])
            let toUsers = ArrayUtils.update(v.toUsers || [], cache.toUsers || [])
        ...
        this.$nextTick(() => {
          this.$refs._page.getScroll().scrollIntoView(this.$refs[`_message_${cache.id}`][0])
        })
        this.$refs._page.getScroll().update()
      }
    },
    methods: {
      getCardType(type) {
      ...
      getCardRef(msgId) {
        return `_message_${msgId}`
      },
      loadMessages() {
        let params = this.getStorage('_message_params_cache', true) || {}
        if (params.msgId) {
          MessageApi.getMessageById(params.msgId).then(res => {
            this.messages = [res.message]
          })
        } else {
          let postData = {
            ...params,
            queryType: params.queryType || 'type',
            currentPage: this.currentPage
          }
          if (params.queryType === 'keyword') {
            this.hasSearch = true
            if (this.keyword === '') {
              postData.queryType = 'type'
            }
      ...
      handleBack() {
        this.getStorage('_message_params_cache')
      },
      mounted() {
        this.loadMessages()
      }
