...
export default {
  ...
  created() {
    ...
    delete params.templetId
    // 图书，图标特殊处理：queryType=templetType的值，noTempletReport =1
    if (this.tab.templetType) {
      params.queryType = this.tab.templetType
      params.noTempletReport = this.tab.noTempletReport || '1'
  ...
  methods: {
    ...
    drawRadar(data) {
      echarts.init(this.$refs[this.tab.name]).setOption({
        radar: {
    ...
    drawLine(data) {
      ...
      echarts.init(this.$refs[this.tab.name]).setOption(option)
    }
  }
}
</script>
