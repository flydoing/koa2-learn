<template>
  <page title="选择我的课程">
    <item v-if="courseGroups.length>0" v-for="(group, g) in courseGroups" :key="g" @click="courseItemClick(group)">
      <div slot="before">
        <img class="logo" v-lazy="{src:url(group.logo,'thumb'),error:Image.ICO_GROUP}" alt='LOGO'>
      </div>
      <div>{{group.name}}</div>
    </item>
  </page>
