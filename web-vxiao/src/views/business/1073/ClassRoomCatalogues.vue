<template>
  <page :title="title">
    <div slot="btn">
      <label @click="changeCourse" v-if="isClass">换课程</label>
    </div>
    <item v-if="catalogues.length>0" v-for="(item, i) in catalogues" :key="i" @click="itemClick(item)">
      <div>{{item.name}}</div>
  ...
  export default {
    ...
    created() {
      ...
      if (this.routeQuery('type') === '1047') {
        reqBody.queryType = '1073'
      } else {
        reqBody.extension = '1073'
      }
