  ...
  <page title="详情">
    <div class="vscreen-detail pd">
      ...
      <div class="vd-sub-title">
        <span class="text-color has-click" v-html="message.categoryName || message.businessName"></span>
        <span class="text-label f12">{{time}}</span>
      </div>
      <div class="vd-content" v-html="content"></div>
      <div class="vd-images">
        <img v-for="(item,i) in message.medias" :item="item" :key="i" v-lazy="{src:toImage(item)}" alt='image' />
      </div>
      <div class="vd-operate-bar">
        <div class="op-left">举报</div>
        <div class="op-right">已阅（{{message.readCount}}）
          <i class="ico ico-heart" @click="handlePraise"></i> {{message.praiseCount}}
        </div>
      </div>
    </div>
