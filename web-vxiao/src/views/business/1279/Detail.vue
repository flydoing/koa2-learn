<template>
  <Page :title="title"
    ...
    <Item v-for="(v,i) in visionPlace.devices"
      ...
      <div slot="remark">{{v.localAddress === '' ? '必选' : v.localAddress}}</div>
    </Item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      activatedFun() {
        let { title, placeId } = this.$route.query
        this.title = title || '详情'
        // if (placeId && !this.visionPlace.id) {
        if (placeId && this.updatePlaceId) {
          this.visionPlace = {
            devices: []
          }
          Api.getVisionPlaceDetail(placeId).then(res => {
            this.visionPlace = res.visionPlaces[0]
          })
        }
    ...
    beforeRouteEnter(to, from, next) {
      ...
      next(vm => {
        if (path.indexOf('1279/device') > -1) {
          vm.updatePlaceId = false
        } else {
          vm.updatePlaceId = true
        }
        vm.activatedFun()
      })
      // next()
