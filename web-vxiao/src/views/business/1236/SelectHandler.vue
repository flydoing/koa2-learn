...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(['setBarter']),
      fetchData() {
        // 判断一进来有没有数据
        Api.tacitlyApprove(1236, this.groupId).then(res => {
          // 判断有没有这个数组
      ...
      handleAdd(type) {
        // 添加  需要判断是审批人还是发货人
        this.setBarter({ user: type === '1' ? this.approves[0] : this.delievers[0], type: type })
        this.routePush({ name: 'selectMember1236' })
      }
    }
  }
</script>
