<template>
  <page :title="title">
    <div class="rating-content">
      ...
      <template v-if="rating.hasAuth && rating.way.type != 'point'">
...
<script>
  ...
  export default {
    ...
    props: {},
    components: {
      RatingItem: resolve => require(['./RatingItem'], resolve)
    },
    data() {
      return {
        ranks: [
    ...
    computed: {
      ...
      canEdit() {
        let hasAuth = this.rating.hasAuth
        let isExpire = this.rating.isExpire
        return hasAuth && !isExpire
      },
      title() {
        let txt = this.sDate ? '评比详情' : '新建评比'
        return this.rating.project ? `${txt} - ${this.rating.project.name}` : `${txt} - ${this.rating.cls.name}`
      },
      stars() {
        let s = []
        if (this.rating.way) {
    ...
    methods: {
      ...
      fetchData() {
        ...
        if (that.rating.project) {
          ...
          Api.loadRatingClassesForProject(that.currentGroup.groupId, project.id, that.sDate).then(res => {
            that.classes = res.statistics.map(c => {
          ...
          Api.loadRatingProjectsForClass(that.rating.cls.groupId, that.sDate).then(res => {
            if (res.ratingRecord) {
              let pMap = new Map()
      ...
      handlePoint(item) {
        ...
        this.routePush({ name: 'addRating1049' })
      },
      ...
      handleAddRemark() {
        ...
        this.setRating({ items: list, curItem: null, isRemark: true })
        this.routePush({ name: 'addRating1049' })
      }
    },
    watch: {}
