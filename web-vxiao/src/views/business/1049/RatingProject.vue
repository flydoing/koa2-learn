<template>
  <page title="评比项目">
    <el-dropdown class="block-element" trigger="click" @command="handleChangeLevel">
      <div class="el-dropdown-link">
        <item>
          <div slot="main">项目层级</div>
          <div v-if="level" slot="remark">{{level.name}}</div>
        </item>
      </div>
      <el-dropdown-menu class="drop-menu" slot="dropdown">
        <el-dropdown-item class="menu-item" v-for="(level,i) in levels" :key="i" :level="level" :command="level">{{level.name}}</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
    <item>
    ...
    <ProjectList v-if="level" :list="list" :level="level.type" v-on:add="projectAdd" v-on:edit="projectEdit" :remarkHandle="remarkTxt"></ProjectList>
  </page>
</template>
<script>
...
export default {
  ...
  components: {
    ...
    ProjectEdit: resolve => require(['./ProjectEdit'], resolve)
  },
  methods: {
    ...mapActions(['setRating', 'modifyMetaData']),
    ...
    fetchData(isUpdate) {
      const that = this
      Api.loadProjectData(that.currentGroup.groupId, isUpdate).then(res => {
        if (res) {
          that.list = res.sets.ratingItems
          let lm = res.sets.levelMetaData
          if (lm) {
            that.levelMeta = lm
    ...
    handleSwitch() {
      ...
      let meta = {
        ...
        metaType: 'm_rating_by_system_set',
    ...
    projectAdd(parent, list, parentIndex, dp) {
      ...
      this.setRating({ parent: parent, title: title, depth: dp, cache: null, level: this.level, project: null })
      this.routePush({ name: 'projectEdit1049' })
    },
    projectEdit(project, parent, index, parentIndex, dp) {
      let title = parentIndex ? `编辑${parentIndex}.${index}项目` : `编辑${index}项目`
      this.setRating({ parent: parent, title: title, depth: dp, cache: project, level: this.level, project: null })
      this.routePush({ name: 'projectEdit1049' })
    }
  }
}
