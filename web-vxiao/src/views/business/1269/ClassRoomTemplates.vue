<template>
  <page title="选择评课模板"
        @scrollEnd="scrollEnd">
    <item v-if="templatelist.length>0"
          v-for="(template, t) in templatelist"
          :key="t"
          @click="itemClick(template)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadTemplates() {
        Api.getClassRoomTemplates(this.currentGroup.groupId, '1269', this.userId, this.currentPage).then(res => {
