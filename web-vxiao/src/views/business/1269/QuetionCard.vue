  ...
  <div class="quetion">
    ...
    <template v-if="quetion.type==='6' || quetion.type==='7'">
      ...
      <div class="quetion-options">
        <item :hasClick="false" v-if="quetion.options.length>0" v-for="(option, o) in quetion.options" :key="o">
    ...
    <template v-else-if="quetion.type==='16'">
      ...
      <div class="quetion-options">
        <div class="option-left left">
          <div v-for="(option, o) in quetion.options.filter(option=>{return option.sort === 1})" :key="o">{{option.content}}</div>
        </div>
        <div class="option-right right">
          <div v-for="(option, o) in quetion.options.filter(option=>{return option.sort === 2})" :key="o">{{option.content}}</div>
        </div>
      </div>
    </template>
    <div class="quetion-counter">
      <span class="is-click" @click.stop="analyClick">查看解析</span>
      <span>出过{{quetion.useCount}}次</span>
      <span>正确率：{{quetion.rightRate}}%</span>
      <span class="delete" v-if="routeQuery('isDelete')" @click="delectQuestion">删除</span>
    </div>
  </div>
    ...
    &-options {
      ...
      .option-right {
        div {
          ...
          max-width: rem(240);
          margin-top: rem(5);
          padding: rem(10);text-align: center;
          border: $border;
        }
      }
      .option {
