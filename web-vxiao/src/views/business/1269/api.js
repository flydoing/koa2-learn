...
export default {
  ...
  loadCommentMsg(groupId, msgId, currentPage = 1) {
    return http.get(`/business/message/query/${groupId}?queryType=parentId&parentId=${msgId}&type=126901&currentPage=${currentPage}`)
  },
  /**
   * 校级 应用设置 ip绑定班级列表
   */
  ...
  getResourceCatalogues({ groupId, ...args }) {
    // return http.get(`/business/course/query/chapter/${groupId}.json?range=${range}&extension=1269&isCount=1&isGlobal=1&isSerial=1`)
