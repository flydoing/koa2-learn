  ...
  export default {
    ...
    created() {
      if (this.meeting.address) {
        this.keyword = this.meeting.address.title
      }
    },
    methods: {
        ...
        geolocation.getCurrentPosition(data => {
          if (data) {
            ...
            geo.getLocation(data.point, result => {
              if (result && !that.keyword) {
                this.keyword = result.addressComponents.city
              }
            })
          }
        })
      ...
      handleAddress(item) {
        let data = { point: item.point, address: item.address, title: item.title }
        this.setMeeting({ address: data })
        this.routeBack()
      }
    }
  }
