...
<script>
  ...
  export default {
    ...
    created() {
      Api.loadMedalListSend(this.currentGroup.groupId).then(res => {
    ...
    methods: {
      ...
      handleAdd(dimension, index) {
        this.setTeacherDate({ dimension: dimension, medal: null })
        if (dimension.description) {
          this.routePush({ name: 'selectMedal1264' })
        } else {
          this.routePush({ name: 'medalCreate1264' })
        }
      },
      handleRemove(dimension, medal, index, pIndex) {
        let that = this
        if (that.isClass && medal.userId !== that.userRelation.userId) {
          that.$message({ message: '学校勋章请联系管理员编辑', type: 'warning' })
          return false
        }
        if (dimension.description === '2' && medal.userId !== that.userRelation.userId) {
      ...
      handleAddDim() {
        this.setTeacherDate({ cache: null, dimension: null })
        this.routePush({ name: 'dimensionEdit1264' })
      },
      handleEditDim(item) {
        this.setTeacherDate({ cache: item, dimension: null })
        this.routePush({ name: 'dimensionEdit1264' })
      }
    }
  }
