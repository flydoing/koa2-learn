  ...
  <Page title="批量绑定">
    ...
    <div v-for="(v,k) in groups"
      ...
      <Item v-for="g in v.items"
        ...
        <div slot="before">
          <i class="ico ico-select"
             :class="{'active':!!checkList[g.groupId]}"></i>
        </div>
        <div>{{g.name}}</div>
      </Item>
    </div>
    ...
    methods: {
      next() {
        ...
        this.$import({
          ...
          templateUrl: `/terminal/vbracelet/exportTemplate/${this.currentGroup.parentId}?classIds=${ids}&cardType =13`,
          callback: (res, file) => {
