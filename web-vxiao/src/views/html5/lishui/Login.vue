      ...
      <div class="header-box">
        <a class="header-logo">
          <img class="header-logo-img"
               src="http://vxiao.cn/img/vxiao_ico.png">
...
<script>
  ...
  export default {
    ...
    methods: {
      onLogin() {
        this.$loading({ target: this.$el })
        this.$jsonp('http://sso.njlsedu.cn/api/jsonp', { callback: window.location.host }).then(_res => {
          if (_res.id) {
            const _uId = _res.id
            Api.login(_uId, null, '/thirdparty/lishui/auth').then(res => {
              Storage.local.set(`${STORAGE.LOCAL_USER_INFO}`, JSON.stringify(res.userData))
              Storage.local.set(`${STORAGE.LOCAL_ACCOUNT}`, _uId)
              clearInterval(this.timer)
              window.location.href = '/html5/vx'
            })
          } else {
            this.$message({ type: 'warning', message: '系统出错，没检验到有效凭证' })
          }
        })
      }
    },
