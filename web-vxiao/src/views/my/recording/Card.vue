<template>
  <Card :message="message">
    <div slot="name">
      <span class="text-color">{{m.name}}</span>
    </div>
    <div slot="point">
    ...
    <div slot="label">
      <span class="text-color">{{m.label}}</span>
    </div>
    ...
    <div slot="down"
      ...
      <el-dropdown trigger="click"
        ...
        <el-dropdown-menu class="drop-menu">
          <el-dropdown-item @click.native.stop="handleCreate(message)">转发</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    ...
    methods: {
      ...
      onClick(message) {
        if (this.message.status !== '8') {
          ...
          this.openBrowser(`/papers/live/vplay?playMp4=${message.msgJson.path}`)
        }
      },
      handleCreate(message) {
        this.setStorage('_cache_forward_message_recording', message)
        this.routePush({
          path: '/vx/user/vrecordForward'
        })
      }
    }
  }
