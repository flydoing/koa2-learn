...
<script>
  ...
  import ItemText from '@/modules/chinamobile/components/item/ItemText'
  ...
  export default {
      ...
      searchGroup() {
        ...
        this.groups.forEach(v => {
          if ((v.isTop === 1 && v.type !== Group.SCHOOL) || v.type === Group.SUBSCRIBE_IMP) {
            ...
            let _metas = this.spaceMeta.filter(_meta => {
    ...
    methods: {
      ...mapActions(['loadGroupUsers', 'currentGroupId', 'modifyGroup', 'modifyNotify', 'saveNotifyTodolist', 'setCurrentApp', 'setMenuType', 'showGroupContacts']),
      rule(g, active = '') {
      ...
      handleGroup(g) {
        ...
        this.setCurrentApp(null)
        this.$emit('click', g, this.searchGroup)
        this.routeReplace({
          path: `/vx`
        })
      },
    ...
    watch: {
      ...
      searchGroup(v) {
        let allBubbleCount = 0
        const that = this
        setTimeout(function() {
          that.searchGroup.forEach(item => {
            if (item.bubbleCount > 0) {
              allBubbleCount = allBubbleCount + item.bubbleCount
            }
          })
          that.$emit('allBubbleCountList', allBubbleCount)
        }, 10)
        if (Check.isNullObject(this.historyGroup) && v[0]) {
          this.currentGroupId(v[0].groupId)
        }
      }
    }
  }
