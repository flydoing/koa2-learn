  ...
  <MenuPage class="mySelfWrapper">
    <template slot="left">
      ...
      <label :class="{'active':index==='self'}"
             @click="handleItem('self')"><img class="self-icon"
             src="/static/ico/chinaMobile/myMessage.png"
             alt="">我的信息</label>
      <label :class="{'active':index==='working'}"
             @click="handleItem('working')"><img class="self-icon"
             src="/static/ico/chinaMobile/currentStatus.png"
             alt="">当前状态</label>
      ...
      <label v-if="self.type==='2'"
             @click="handelTeacherInfo"><img class="self-icon"
             src="/static/ico/chinaMobile/teacherDev.png"
             alt="">教师发展档案</label>
      <label v-for="(u,i) in self.relations"
             :key="i">
        <span><img class="self-icon"
               src="/static/ico/chinaMobile/studentMsg.png"
               alt="">学生成长</span>
        <span>{{u.name}}</span>
      </label>
      <label @click="onQuit"><img class="self-icon"
             src="/static/ico/chinaMobile/logout.png"
             alt="">退出登录</label>
    </template>
  </MenuPage>
</template>
<script>
  import Api from '@/views/my/api'
