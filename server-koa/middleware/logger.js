...
module.exports.loggerAccess = async (ctx, next) => {
  ...
  DatabaseLog.insertLogAccess(`--> request: ${client}`, responseTime, response, body)
}
module.exports.loggerError = async (err, ctx) => {
  ...
  // 入库
  DatabaseLog.insertLogError(`--> request: ${client}`, err)
}
module.exports.loggerErrorTextLog = async (err) => {
  const loggerError = log4js.getLogger('error')
  console.log('------> loggerErrorTextLog')
  loggerError.error(err)
}
